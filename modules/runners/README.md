...
| [aws_lambda_layer_version.barracuda_ca](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lambda_layer_version) | resource |
...
| <a name="input_http_proxy"></a> [http\_proxy](#input\_http\_proxy) | HTTP proxy URL for scale-up and scale-down Lambda functions. Configure the EC2 runner proxy through userdata\_pre\_install. Embedded credentials are not allowed. | `string` | `null` | no |
...
| <a name="input_no_proxy"></a> [no\_proxy](#input\_no\_proxy) | Comma-separated list of hosts and domains excluded by the scale-up and scale-down Lambda proxy configuration. | `string` | `null` | no |
...
| <a name="input_restricted_github"></a> [restricted\_github](#input\_restricted\_github) | When true, the SSM housekeeper Lambda skips VPC placement. | `bool` | `true` | no |
