...
variables {
  config = {
    ...
    webhook_allowed_source_cidrs = ["192.30.252.0/22"]
...
run "plan_without_vpc" {
  ...
  assert {
    ...
    error_message = "Empty network lists should omit webhook VPC placement and preserve the source allowlist"
  }
}
...
run "plan_with_vpc" {
  ...
  assert {
    ...
    error_message = "Configured network lists should preserve webhook VPC placement and permissions"
  }
}
...
run "plan_with_partial_vpc_configuration" {
  ...
  assert {
    ...
    error_message = "Partial VPC configuration should not place the webhook in a VPC or grant ENI permissions"
  }
}
